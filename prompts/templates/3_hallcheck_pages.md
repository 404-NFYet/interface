...
2. 아래 항목을 모두 검사해요:
   ...
   - **Quiz 검증**:
     - `quiz.question`과 `quiz.options`가 `historical_case`의 사실 관계와 일치하는지
     - `quiz.correct_answer`가 논리적으로 타당한지
     - `quiz.lesson`이 본문의 교훈과 부합하는지
...
3. `validated_pages`의 각 페이지는 `step`, `title`, `purpose`, `content`, `bullets`, `quiz`(Step 3 필수) 필드를 포함해요.
...
```json
{
  ...
  "issues": [
    {
      "component": "theme|one_liner|pages|quiz",
      "field_path": "string",
      "claim": "string",
  ...
  "validated_pages": [
    {
      ...
      "quiz": {
        ...
        "question": "string",
        "options": [
          { "id": "string", "label": "string", "explanation": "string" }
        ],
