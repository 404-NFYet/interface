...
- **Quiz 생성 (필수)**: 독자가 과거 사례의 교훈을 맞출 수 있는 3지선다 퀴즈를 `quiz` 필드에 생성해요.
  - `context`: 퀴즈 풀기에 필요한 배경 상황 요약
  - `question`: 독자에게 던지는 질문
  - `options`: 3개 선택지 (정답 1개, 오답 2개). 각 선택지는 `id`, `label`, `explanation` 포함.
  - `correct_answer`: 정답 선택지의 `id`
  - `actual_result`: 실제 역사적 결과 설명
  - `lesson`: 이 사례에서 얻을 수 있는 교훈
