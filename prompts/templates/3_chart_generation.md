...
description: "Chart Agent Step 2: Generation"
---
...
3. **출처 표기 (Cite Sources)**:
   - 데이터가 어디서 왔는지 식별하십시오 (예: "DART (삼성전자 2024 사업보고서)", "한국은행 ECOS").
   - 출처 목록을 다음 형식의 객체 배열로 작성하십시오: `{"name": "출처명", "url_domain": "source.com 또는 'api'", "used_in_pages": [{{ step }}]}`.
...
```json
{
  ...
  "sources": [
    ...
    { "name": "네이버 금융", "url_domain": "finance.naver.com", "used_in_pages": [{{ step }}] }
  ]
}
```

# Constraint (제약 사항)
- 마크다운 코드 블록(```json ...) 없이 **순수 JSON 객체**만 출력하십시오.
- JSON 문법이 유효해야 합니다.
